...
class SkypeMsg(SkypeObj):
    ...
    def quote(user, chat, timestamp, content):
        ...
        chatId = chat.id if chat.id.split(
            ":")[0] == "19" else SkypeUtils.noPrefix(chat.id)
        # Legacy timestamp includes the date if the quote is not from today.
        unixTime = int(time.mktime(timestamp.timetuple()))
        legacyTime = timestamp.strftime("{0}%H:%M:%S".format(
            "" if timestamp.date() == date.today() else "%d/%m/%Y "))
    ...
    def read(self):
        ...
        """
        self.chat.setConsumption("{0};{1};{0}".format(
            self.clientId, int(time.time() * 1000)))
...
class SkypeTextMsg(SkypeMsg):
    ...
    def plain(self):
        if self.content is None:
            return None
        text = re.sub(
            r"</?(e|b|i|ss?|pre|quote|legacyquote)\b.*?>", "", self.content)
...
class SkypeContactMsg(SkypeMsg):
    ...
    def rawToFields(cls, raw={}):
        ...
        contactTags = BeautifulSoup(
            raw.get("content"), "html.parser").find_all("c")
...
class SkypeLocationMsg(SkypeMsg):
    ...
    attrs = SkypeMsg.attrs + ("latitude", "longitude",
                              "altitude", "speed", "course", "address", "mapUrl")

    @classmethod
    def rawToFields(cls, raw={}):
        fields = super(SkypeLocationMsg, cls).rawToFields(raw)
        locTag = BeautifulSoup(
            raw.get("content"), "html.parser").find("location")
...
class SkypeCardMsg(SkypeMsg):
    ...
    def rawToFields(cls, raw={}):
        ...
        swiftTag = BeautifulSoup(
            raw.get("content"), "html.parser").find("swift")
    ...
    def html(self):
        ...
        if self.chatId:
            ...
            if userType == "19":
                ...
                data["recipient"]["name"] = str(
                    self.skype.contacts[userId].name)
...
class SkypeFileMsg(SkypeMsg):
    ...
    class File(SkypeObj):
        ...
        def urlAsm(self):
            ...
            if not self.urlFull:
                return None
            match = re.match(
                "{}/0-([a-z0-9]+)-".format(SkypeConnection.API_ASM), self.urlFull)
            if not match:
                return self.urlFull
    ...
    def rawToFields(cls, raw={}):
        ...
        file = BeautifulSoup(raw.get("content"),
                             "html.parser").find("uriobject")
        if file:
    ...
    def html(self):
        if not self.file:
            return ""
        tag = makeTag("URIObject", type="File.1",
                      uri=self.file.urlFull, url_thumbnail=self.file.urlThumb)
        tag.append(makeTag("Title", "Title: {0}".format(
            self.file.name) if self.file.name else ""))
        tag.append(makeTag("Description", "Description: {0}".format(
            self.file.name) if self.file.name else ""))
...
class SkypeImageMsg(SkypeFileMsg):
    ...
    def html(self):
        if not self.file:
            return ""
        tag = makeTag("URIObject", type="Picture.1",
                      uri=self.file.urlFull, url_thumbnail=self.file.urlThumb)
...
class SkypeCallMsg(SkypeMsg):
    ...
    def rawToFields(cls, raw={}):
        ...
        listTag = BeautifulSoup(
            raw.get("content"), "html.parser").find("partlist")
        ...
        for partTag in listTag.find_all("part"):
            fields["userIds"].append(
                SkypeUtils.noPrefix(partTag.get("identity")))
...
class SkypePropertyMsg(SkypeMsg):
    ...
    def tagTemplate(self, root):
        ...
        timestamp = self.time or datetime.now()
        tag = makeTag(root)
        tag.append(makeTag("eventtime", str(
            int(time.mktime(timestamp.timetuple())))))
        tag.append(makeTag("initiator", "8:{0}".format(
...
class SkypeTopicPropertyMsg(SkypePropertyMsg):
    ...
    def rawToFields(cls, raw={}):
        ...
        propInfo = BeautifulSoup(
            raw.get("content"), "html.parser").find("topicupdate")
        if propInfo:
...
class SkypeOpenPropertyMsg(SkypePropertyMsg):
    ...
    def rawToFields(cls, raw={}):
        ...
        propInfo = BeautifulSoup(raw.get("content"), "html.parser").find(
...
class SkypeAddMemberMsg(SkypeMemberMsg):
    ...
    def rawToFields(cls, raw={}):
        ...
        memInfo = BeautifulSoup(
            raw.get("content"), "html.parser").find("addmember")
        if memInfo:
    ...
    def html(self):
        ...
        tag.append(makeTag("target", "8:{0}".format(
            self.memberId) if self.memberId else ""))
        return tag
...
class SkypeChangeMemberMsg(SkypeMemberMsg):
    ...
    def rawToFields(cls, raw={}):
        ...
        memInfo = BeautifulSoup(
            raw.get("content"), "html.parser").find("roleupdate")
        if memInfo:
            fields.update({"userId": SkypeUtils.noPrefix(memInfo.find("initiator").text),
    ...
    def html(self):
        ...
        valTag.append(makeTag("id", "8:{0}".format(
            self.memberId) if self.memberId else ""))
...
class SkypeRemoveMemberMsg(SkypeMemberMsg):
    ...
    def rawToFields(cls, raw={}):
        ...
        memInfo = BeautifulSoup(
            raw.get("content"), "html.parser").find("deletemember")
        if memInfo:
            fields.update({"userId": SkypeUtils.noPrefix(memInfo.find("initiator").text),
                           "memberId": SkypeUtils.noPrefix(memInfo.find("target").text)})
        return fields
