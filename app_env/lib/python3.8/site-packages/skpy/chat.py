...
class SkypeChat(SkypeObj):
    ...
    def getMsgs(self):
        ...
        """
        url = "{0}/users/ME/conversations/{1}/messages".format(
        ...
        resp = self.skype.conn.syncStateCall(
            "GET", url, params, auth=SkypeConnection.Auth.RegToken, headers=headers).json()
    ...
    def editRaw(self, msgId, msg):
        ...
        if "errorCode" in json:
            raise SkypeApiException(
                "Error code {0} from message edit".format(json["errorCode"]), resp)
        else:
    ...
    def deleteRaw(self, msgId):
        ...
        if "errorCode" in json:
            raise SkypeApiException(
                "Error code {0} from message delete".format(json["errorCode"]), resp)
        else:
    ...
    def sendRaw(self, editId=None, **kwargs):
        ...
        arriveDate = datetime.fromtimestamp(
            arriveTime / 1000) if arriveTime else datetime.now()
        msg.update({"id": msgId,
        ...
        if clientTime:
            ...
            msg["composetime"] = datetime.strftime(
                clientDate, "%Y-%m-%dT%H:%M:%S.%fZ")
        if arriveTime:
            arriveDate = datetime.fromtimestamp(arriveTime / 1000)
            msg["originalarrivaltime"] = datetime.strftime(
    ...
    def sendContacts(self, *contacts):
        ...
        """
        contactTags = ("""<c t="s" s="{0}" f="{1}"/>""".format(
            contact.id, contact.name) for contact in contacts)
    ...
    def setConsumption(self, horizon):
        ...
        self.skype.conn("PUT", "{0}/users/ME/conversations/{1}/properties".format(self.skype.conn.msgsHost, self.id),
                        auth=SkypeConnection.Auth.RegToken, params={
...
class SkypeGroupChat(SkypeChat):
    ...
    attrs = SkypeChat.attrs + ("topic", "creatorId", "userIds",
                               "adminIds", "open", "history", "picture", "active")
...
class SkypeChats(SkypeObjs):
    ...
    def recent(self):
        ...
        resp = self.skype.conn.syncStateCall(
            "GET", url, params, auth=SkypeConnection.Auth.RegToken).json()
        chats = {}
    ...
    def create(self, members=(), admins=()):
        ...
        """
        memberObjs = [{"id": "8:{0}".format(
            self.skype.userId), "role": "Admin"}]
        for id in members:
            if id == self.skype.userId:
                continue
            memberObjs.append({"id": "8:{0}".format(
                id), "role": "Admin" if id in admins else "User"})
    ...
    def urlToIds(url):
        ...
        json = SkypeConnection.externalCall(
            "POST", convUrl, json={"shortId": urlId, "type": "wl"}).json()
